...
export const dashboardController = {
  ...
  async getIntegrations(req: ExpressRequest, res: Response) {
    try {
      ...
      // Fetch both integrations and onboarding status
      const [integrations, onboarding] = await Promise.all([
        prisma.integration.findMany({
          where: { userId: req.user.id },
          select: { id: true, platform: true, status: true, lastSync: true, errorLog: true }
        }),
        prisma.onboardingStatus.findUnique({
          where: { userId: req.user.id }
        })
      ]);

      res.json({ integrations, onboarding });
    } catch (error) {
  ...
  async disconnectIntegration(req: ExpressRequest, res: Response) {
    try {
      if (!req.user) {
        res.sendStatus(401);
        return;
